...
const Conferences = () => {
  ...
  const handleRegisterYes = async () => {
    ...
    if (courses) {
      ...
      const emailData = {
        email: userMail,
        content: `Registration for ${courses.name} - ${courses.description}. Price: ${courses.price}`,
      };
      switch (courses.abv) {
        case "co":
          try {
            
            dispatch(engageToCourse(courses.id, userId));
      ...
      {courses && (
        <>
          <div className="the-header-in-conferences">
            ...
            <div>
              <h2 className="the-Our-Conferences">{courses.languageName}{courses.level}</h2>
            </div>
          </div>
       
            
           
            <div key={courses.id} className="ConferencePage" >
              <div className="ConferencePageimg" ><img src={courses.img}/></div>
                <div className="conference-side">
                  <h2 className="header-in-thee-slide">About</h2>
                  <p>Translation and language courses in the United Kingdom are renowned platforms for professionals to come together to discuss, share, and learn about the latest trends, techniques, and developments in the field. These courses provide a space for networking, knowledge exchange, and skill enhancement among professionals who are passionate about languages and translation.</p>
                  <p>{courses.description}</p>
                  <p>One of the key objectives of these courses is to foster collaboration and dialogue among professionals from different parts of the world. This helps to stimulate innovation and promote best practices in the field of translation and languages.

                    Overall, these courses play a crucial role in shaping the future of translation and languages in the UK. They provide a platform for professionals to learn from each other, share their experiences, and contribute to the ongoing development of these fields</p>
                
                  <p className="header-in-thee-slide">{courses.price}</p>
